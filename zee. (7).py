...
def get_title(predictions, categories, first_line=''):
    txt = f"{first_line}\nDetected emotion:   {categories[predictions.argmax()]} - {predictions.max() * 100:.2f}%"
    return txt
...
def plot_melspec(path, tmodel=None, three=False,
    ...
    if tmodel is None:
        tmodel = load_model("D:/speech-emotion-webapp-master/speech-emotion-webapp-master/tmodel_all.h5")
...
# load models
model = load_model("D:/speech-emotion-webapp-master/speech-emotion-webapp-master/model3.h5")
...
def main():
    side_img = Image.open("D:/speech-emotion-webapp-master/speech-emotion-webapp-master/images/emotion3.jpg")
    with st.sidebar:
    ...
    if website_menu == "Emotion Recognition":
        ...
        with col1:
            st.info("alenroy404@gmail.com")

        with col2:
            liimg = Image.open("D:\speech-emotion-webapp-master\speech-emotion-webapp-master\images\LI-Logo.png")
            st.image(liimg)
            st.markdown(f""":speech_balloon: [Alen Roy](https://www.linkedin.com/in/alen-roy/)""")
